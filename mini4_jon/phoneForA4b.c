...
int loadCSV(struct PHONE_RECORD *phonebook, int size) {
    char array[1000];
...
int saveCSV(struct PHONE_RECORD *phonebook, int size) {
...
int addRecord(struct PHONE_RECORD *phonebook, struct PHONE_RECORD newRecord, int size) { // 
    int i = 0;
    int filled = 0;
    while (i < size) {
...
int findRecord(struct PHONE_RECORD *phonebook, char nameFind[50], int size) {
    printf("Find name: %s\n", nameFind);
    for (int i = 0; i < size; i++) {
...
int listRecord(struct PHONE_RECORD *phonebook, int size) {
